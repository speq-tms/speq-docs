...
SPEQ может находиться непосредственно в репозитории с кодом. Это даёт прямой доступ к Swagger/OpenAPI-документации проекта без внешних сервисов. Coverage-отчёт отвечает на вопрос: "Какие эндпоинты у нас протестированы, а какие нет?" — и является естественным продолжением ATDD-флоу (doc 22): pending-тесты вносят вклад в coverage, а непокрытые эндпоинты становятся очевидными кандидатами для новых тестов.
