...
> строковых значениях `properties`.
...
1. **Окружение.** Каждый ключ файла окружения, кроме `baseUrl`, становится переменной с тем же именем.
   Сам `baseUrl` доступен как `{{baseUrl}}`.
...
### Неизвестное имя не является ошибкой

```yaml
url: "/posts/{{нетТакойПеременной}}"
```

`render_template` оставляет `{{нетТакойПеременной}}` в строке как есть и отправляет запрос с этим текстом
в URL. Ни ошибки, ни предупреждения не будет — только неожиданный ответ сервера. Это самая частая причина
загадочных `404` (см. [speq-tms/speq-docs#27](https://github.com/speq-tms/speq-docs/issues/27)).
...
**Результат шага нельзя передать в следующий шаг внутри тест-спеки.** Поле `id` у шага `api` работает
только внутри модуля — для выражений `returns`. В тесте `{{created.response.body.id}}` ни к чему не
привязано и подставится буквально. Единственный способ передать значение между шагами — действие модуля
с `returns` и `as`. Отслеживается в [speq-tms/speq-docs#27](https://github.com/speq-tms/speq-docs/issues/27).

**Нераскрытая подстановка не является ошибкой.** `{{...}}` с неизвестным именем уезжает в запрос как есть.

**К элементам массива обратиться нельзя** — ни в `path` ассертов, ни в `condition.path`, ни в выражениях
`returns`.

**Заголовки окружения не отправляются.** Ключ `headers` в файле окружения становится обычной переменной;
на запрос влияют только `headers` самого шага
([speq-tms/speq-docs#21](https://github.com/speq-tms/speq-docs/issues/21)).
