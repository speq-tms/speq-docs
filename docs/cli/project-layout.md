...
Справочник самого DSL — в [dsl.md](dsl.md). Первый зелёный прогон с нуля — в [quickstart.md](quickstart.md).
...
`discover_speq_root` (`src/cli/discovery.rs`) работает так:

1. Если передан `--speq-root <path>` — используется он, и поиск не выполняется вовсе. Относительный
   путь разрешается от текущего каталога. Режим в выводе команд при этом обозначается как `explicit`,
   а существование каталога на этом шаге не проверяется.
2. Иначе CLI идёт от текущего каталога вверх по дереву — как `git`, `cargo` или `npm` — и берёт
   **первый** каталог, похожий на проект. В каждом каталоге по пути:
   - есть `.speq/manifest.yaml` и `.speq/suites/` — это `in-repo`, корень `<каталог>/.speq`;
   - есть `manifest.yaml` и `suites/` — это `test-repo`, корень `<каталог>`.

Побеждает ближайший корень. Если проект вложен в другой проект, команда из внутреннего работает с
внутренним; внешний не рассматривается вовсе.

Если до корня файловой системы ничего не нашлось:

```text
speq root not found in <каталог> or any parent directory; run 'speq init' or pass --speq-root
```

Если в **одном** каталоге сработали обе проверки (есть и `manifest.yaml` + `suites/`, и
`.speq/manifest.yaml` + `.speq/suites/`), CLI отказывается угадывать и называет этот каталог — при
поиске вверх он не обязан совпадать с текущим:

```text
ambiguous speq layout in <каталог>: both .speq and repository root look valid, pass --speq-root
```

Неоднозначность останавливает поиск только там, где она найдена. Если ближе по пути есть нормальный
корень, он и используется — до неоднозначного предка дело не доходит.

Признак корня — именно пара «манифест + каталог сьют». Одного `manifest.yaml` недостаточно.

> Каталог `.speq` сам удовлетворяет форме `test-repo` — в нём лежат `manifest.yaml` и `suites/`.
> Поэтому запуск изнутри `.speq` находит тот же корень и сообщает режим `in-repo`, а не `test-repo`.
...
Ключ `baseUrl` задаёт префикс относительных URL, `headers` — заголовки, добавляемые к каждому запросу
прогона, любой другой ключ верхнего уровня становится переменной с тем же именем — см.
[dsl.md](dsl.md#переменные-и-подстановка) и [Заголовки окружения](dsl.md#заголовки-окружения).
...
- [dsl.md](dsl.md) — полный справочник DSL: тесты, шаги, ассерты, переменные, генерация, модули, фикстуры.
